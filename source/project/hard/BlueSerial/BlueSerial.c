...
void Blue_Slider_Control(void)
{
	if (BlueSerial_ReceiveFlag())    //判断是否收到蓝牙数据包
	{
		...
		if (strcmp(BlueSerial_StringArray[0], "slider") == 0)//
		{
			...
			if(strcmp(BlueSerial_StringArray[1],"distance") == 0)distance = atoi(BlueSerial_StringArray[2]);
			...
			/* [slider,dist,50] 循迹定距50cm停车 */
			else if(strcmp(BlueSerial_StringArray[1],"dist") == 0)
			{
			    int32_t dist_cm = atoi(BlueSerial_StringArray[2]);
			    Run_To_Distance(dist_cm);
			}
		}
	}
}
